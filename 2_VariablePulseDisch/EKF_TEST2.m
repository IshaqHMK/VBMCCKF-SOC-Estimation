...
%% Data:
global Q R
global Data_SOC Data_OCV Current Voltage k_steps Cn N Time TDiff SOC_TL OCV_TL
...
Q =  0.01;          % Sensor-noise covariance
...
xhat0 =   0.9;
% EKF works well for samaller xhat
iR10  =   0;
Hk0   =   0;
...
for k = 2 : k_steps
   ...
   % Compute OCV
   OCV(k) = interp1(SOC_TL,OCV_TL, state(1,1,k) ,'linear','extrap');
...
EKF3 = figure;
plot(Time, RMSE_EKF,'k-' ,'LineWidth',2 ); grid;
legend('RMSE','location','northeast');
title('RMSE - EKF'); xlabel('TimeSteps'); ylabel('RMSE');
Name  = [num2str(KF),'RMSE'];
print(EKF3,Name,FType);

fprintf('The RMS SOC Estimation Error = %g%%\n',sqrt(mean((100*(xTruth-xEstimate)).^2)));