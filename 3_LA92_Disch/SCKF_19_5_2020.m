...
Q =  diag([1e-12 1e-3 1e-3]);           % Process Noise covariance
...
for k = 2:k_steps
   ...
   Xi = repmat(state(:,:,k),1,m) + Sk(:,:,k)*CubaturePoints;
...
%  H(1,1,k) = polyval(pd,state(1,1,k));
   ...
   %Insrt
   [foo,Sz] = qr([Z Rsqrt]',0);
   Sz = Sz';    % Square-root Innovations Covariance
